                               ...
                               public void onFinish() {
                                ...
                                //Caso o cadastro seja sucedido
                                     ...
                                     new CountDownTimer(1500, 200) {
                                         ...
                                         @Override
                                         public void onFinish() {
                                             if(radioButton.getText().toString().equals("Doar")){
                                                 String userId = mAuth.getCurrentUser().getUid();
                                                 DatabaseReference currentUserDb1 = FirebaseDatabase.getInstance().getReference().child("Users").child(radioButton.getText().toString()).child(userId).child("Nome Pessoa");
                                                 currentUserDb1.setValue(nomecompleto);
                                                 Intent intent = new Intent(CadastrarActivity.this, CadastropetActivity.class);
                                                 startActivity(intent);

                                             } else if(radioButton.getText().toString().equals("Adotar")){
                                                 String userId = mAuth.getCurrentUser().getUid();
                                                 DatabaseReference currentUserDb2 = FirebaseDatabase.getInstance().getReference().child("Users").child(radioButton.getText().toString()).child(userId).child("nome");
                                                 currentUserDb2.setValue(nomecompleto);
    ...
    private void userProfile()
    {
        ...
        if(user!= null)
        {
            UserProfileChangeRequest profileUpdates = new UserProfileChangeRequest.Builder()
                    .setDisplayName(vtxNomeCompleto.getText().toString().trim())
